...
client.on("message", async message => {
    ...
    if (!message.content.startsWith(prefix) && message.content.includes(client.user.id))
        if (!message.guild.me.permissionsIn(message.channel).has("EMBED_LINKS"))
            message.reply(new Discord.MessageEmbed().setColor(config.colors.yes).setAuthor(`${message.author.username}, Az én prefixem a következő: ${prefix}, kezdésnek használd a:${prefix}help-et!`, message.author.displayAvatarURL({
                dynamic: true
            })));
        else
            message.reply(`${message.author.username},Az én prefixem a következő: ${prefix}, kezdésnek használd a:${prefix}help`)
    if (!message.content.startsWith(prefix)) return;

    //if not allowed to send embeds, return that
    if (!message.guild.me.permissionsIn(message.channel).has("EMBED_LINKS"))
        return message.reply("**:x: Nincs hozzá jogod,hogy végre tud hajtani a parancsot `EMBED_LINKS`**")
    ...
    if (client.settings.get(message.guild.id, `botchannel`).toString() !== "") {
        if (!client.settings.get(message.guild.id, `botchannel`).includes(message.channel.id) && !message.member.hasPermission("ADMINISTRATOR")) {
            ...
            return functions.embedbuilder(client, 5000, message, config.colors.no, `Nincs bot chat!`, `Ebben a GUILD-ben van egy Bot chat beállítás!Próbáld meg itt használni a Bot parancsokat: 
            > ${leftb}`)
        }
    }
    ...
    if (command) {
        ...
        if (timestamps.has(message.author.id)) {
            ...
            if (now < expirationTime) {
                ...
                return message.reply(
                    `Kérlek várj: ${timeLeft.toFixed(1)}mpt és utána használd a(z) \`${command.name}\` prancsot!`
                );
            }
        }
        ...
        message.react("✅").catch(e => console.log("Nem kell reagálni"))

        try {
            command.run(client, message, args);
        } catch (error) {
            console.error(error)
            functions.embedbuilder(client, 5000, message, "RED", "ERROR: ", "```" + error.toString().substr(0, 100) + "```" + "\n\n**Error lépet fel! Kérlek szólj a BOT készitőjének**")
            functions.errorbuilder(error.stack.toString().substr(0, 2000))
        }
    } else
        return message.reply(`Helytelen parancs!Használd: ${prefix}help`)
});
